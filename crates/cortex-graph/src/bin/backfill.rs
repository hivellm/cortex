...
async fn main() -> Result<()> {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new(format!("{level},cortex_graph={level}")));
    ...
    if cli.dump_first_patch {
        if let Some(ev) = scan.events.first() {
            let patch = cortex_graph::map_event_to_patch(ev);
            println!("=== first event ===");
...
/// extraction rules as
/// `cortex_classifier_worker::worker::ClassifiedEvent::from_canonical_envelope`.
...
fn render_node_merge_for_print(node: &cortex_graph::NodeOp) -> String {
