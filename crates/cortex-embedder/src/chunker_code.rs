...
mod tests {
    ...
    fn rust_file_with_ten_top_level_fns_produces_ten_chunks() {
        // Generate ten short fns with unique bodies to keep each under the
        // 8 KB threshold.
        ...
        for i in 0..10 {
            ...
            // Pad the body so the whole file is ~1000 lines.
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            ...
            // Round-trip: the chunk text is exactly the substring of the
            // source at its declared byte range.
            assert_eq!(
                chunk.text,
    ...
    fn oversize_rust_fn_is_windowed() {
        ...
        for chunk in &chunks {
            ...
            let (start, end) = chunk.metadata.byte_range.expect("byte range present");
            assert!(end > start);
            // Byte ranges were rebased onto the original document.
