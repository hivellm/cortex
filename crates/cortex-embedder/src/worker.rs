...
use crate::embedder::{EmbedError, EmbedReport, Embedder, EnrichedEvent};
...
mod tests {
    use super::*;
    use crate::embedder::VectorizerEmbedder;
    use crate::vectorizer_client::{
        CollectionSchema, MemoryCall, MemoryVectorizerClient, UpsertReport,
        VectorizerClient, VectorizerClientError,
    };
    use async_trait::async_trait;
    ...
    async fn happy_path_publishes_embedded_for_each_event() {
        ...
        // Vectorizer saw 3 upsert calls (one per event, since each event's
        // chunks fit in a single batch).
    ...
    // ---- Rate-limited client stub used by the backpressure test --------
    ...
    async fn rate_limit_arms_backpressure_and_halts_batch() {
        ...
        enqueue_event(&consumer, 0, &event);
        enqueue_event(&consumer, 1, &event); // will not be reached

        let _ = worker.run_once().await.unwrap();

        // First message hit the rate-limit, worker halted the batch.
        assert!(worker.backpressure.is_active());
        // Second message is still queued because the batch was halted.
        assert_eq!(consumer.remaining(), 0, "batch drained in one fetch");
        // No invalid envelope for the rate-limit case — it's retriable.
        assert!(publisher.calls_on(STREAM_INVALID).is_empty());
        assert!(publisher.calls_on(STREAM_EMBEDDED).is_empty());

        // Soak threshold not yet reached.
        assert!(!worker.backpressure.is_paused());

        // Force the rate-limit to have been ongoing for 31 s and re-check.
        worker
            .backpressure
            .force_since(std::time::Instant::now() - Duration::from_secs(31));
        assert!(worker.backpressure.is_paused());

        // While paused, run_once is a no-op.
        let handled_while_paused = worker.run_once().await.unwrap();
        assert_eq!(handled_while_paused, 0);

        // Success clears backpressure.
    ...
    async fn replayed_event_is_deduped_by_inmemory_guard() {
        ...
        // Only the first delivery triggers an embed — the second short-
        // circuits via the in-memory processed-id set.
        let upsert_calls = vec_client
            .calls()
            .into_iter()
            .filter(|c| matches!(c, MemoryCall::Upsert(_, _)))
        ...
        // Only one success envelope is published (the second delivery is
        // acked silently without re-publishing).
        let embedded = publisher.calls_on(STREAM_EMBEDDED);
