...
use crate::chunker_code::{detect_language_from_path, CodeChunker};
use crate::chunker_doc::DocChunker;
...
/// Hex-encoded SHA-256 digest of the input. Re-exported for tests that need
/// to match against the summary-substituted chunk id.
#[cfg(test)]
...
mod tests {
    ...
    use crate::vectorizer_client::{MemoryCall, MemoryVectorizerClient};
    ...
    async fn mixed_kind_events_route_to_correct_collections() {
        ...
        // Verify each collection was visited via the memory client log.
    ...
    async fn oversize_with_summary_produces_summary_plus_raw_oversize() {
        ...
        // Markdown with a single section whose body exceeds 4 KB. The
        // DocChunker emits one section chunk whose text crosses the
        // summary-substitution threshold.
    ...
    async fn oversize_without_summary_raises_embed_error() {
        ...
        // Same shape as the positive test but with no classifier summary —
        // surfaces `OversizeWithoutSummary` and drops the event.
        let big_body = "lorem ipsum dolor sit amet consectetur adipiscing. ".repeat(200);
        let md = format!("# Big section\n{body}\n", body = big_body);
    ...
    async fn empty_payload_is_skipped() {
        ...
        // Empty payload: both `content` field and whole JSON collapse to
        // whitespace-only.
    ...
    async fn large_batch_splits_upserts_correctly() {
        ...
        // Synthesise 150 top-level Rust fns so a single event yields 150
        // Code chunks — 64 + 64 + 22 == 3 upsert calls.
