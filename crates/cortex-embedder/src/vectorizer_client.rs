...
pub struct UpsertedChunk {
    /// Client-side deterministic dedup key (from [`crate::identity::dedup_key`]).
...
fn flatten_chunk_metadata(chunk: &Chunk) -> Vec<(String, String)> {
    ...
    out.push((
        ...
        match chunk.metadata.source {
            crate::chunker::ChunkSource::Code => "code".into(),
            crate::chunker::ChunkSource::Doc => "doc".into(),
            crate::chunker::ChunkSource::Summary => "summary".into(),
            crate::chunker::ChunkSource::FallbackWindow => "fallback_window".into(),
            crate::chunker::ChunkSource::RawOversize => "raw_oversize".into(),
        },
    ));
...
mod tests {
    use super::*;
    use crate::chunker::{ChunkMetadata, ChunkSource};
