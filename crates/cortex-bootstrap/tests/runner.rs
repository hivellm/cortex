...
"#;
    let parsed: cortex_bootstrap::CortexToml = toml::from_str(body).unwrap();
    parsed.cortex
}
...
async fn parallel_runner_executes_all_repos_with_no_event_loss() {
    use cortex_bootstrap::run_repos_parallel;
...
async fn checkpoint_persists_atomically_to_disk() {
    ...
    cortex_bootstrap::write_atomic(&cp_path, &checkpoint).unwrap();
    let reloaded = cortex_bootstrap::load_checkpoint(&cp_path).unwrap();
...
async fn drops_oversize_files_and_records_drop_reason() {
    ...
    let big = vec![b'x'; (cortex_bootstrap::MAX_FILE_BYTES as usize) + 1024];
    fs::write(&big_path, &big).unwrap();
    let cfg = fixture_config();
    let entries = cortex_bootstrap::walk_repo(repo.path(), &cfg);
    assert!(
        entries.iter().any(|e| matches!(
            e,
            cortex_bootstrap::WalkEntry::Dropped { reason: "oversize", .. }
        )),
...
fn classify_path_via_public_api() {
    ...
    assert_eq!(
        cortex_bootstrap::classify_path("docs/decisions/0042/intro.md", &cfg),
        cortex_bootstrap::FileClass::Decision
    );
    assert_eq!(
        cortex_bootstrap::classify_path("src/lib.rs", &cfg),
        cortex_bootstrap::FileClass::Code
    );
}

#[test]
fn estimate_format_string_includes_every_sizing_line() {
    let est = cortex_bootstrap::Estimate {
        repo_id: "Demo".into(),
        files_kept: 10,
        files_dropped: 2,
        code_chunks_est: 100,
    ...
    let out = cortex_bootstrap::format_estimate(&est);
    for needle in [
        "Repo: Demo",
