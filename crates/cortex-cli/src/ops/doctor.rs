...
//!   `(repo_slug, family)` using `cortex_workers::fulltext::routing` so the
//!   partition view exactly matches what the live indexer produces.
//! - [`MeiliCoverageProbe`] consumes any
//!   `cortex_workers::fulltext::MeiliClient` (the same trait the live worker
...
use cortex_workers::fulltext::routing::{family_for_event, is_canonical_index_name, FAMILIES};
use cortex_workers::fulltext::MeiliClient;
...
pub struct DoctorReport {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub queries: Vec<super::probe::QueryReport>,
...
/// `cortex_workers::fulltext::MeiliClient` so tests can drive it through
/// `MemoryMeiliClient`.
...
pub fn render_coverage_markdown(report: &DoctorReport) -> String {
    ...
    for q in &report.queries {
        out.push_str(&super::probe::render_query_markdown(q));
    }
...
impl LiveNexusCoverageProbe {
    /// Build a Live probe from a [`cortex_workers::graph::GraphConfig`] (the
    /// caller is expected to pull the config from env via
    /// `GraphConfig::from_env`).
    pub fn new(config: cortex_workers::graph::GraphConfig) -> anyhow::Result<Self> {
...
mod tests {
    use super::*;
    use cortex_workers::fulltext::MemoryMeiliClient;
