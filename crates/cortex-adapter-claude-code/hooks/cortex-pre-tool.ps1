...
Add-Content -Path $logPath -Value "$ts PreToolUse env_sid=$session payload_sid=$payloadSession pid=$PID" -ErrorAction SilentlyContinue
$frame = "{`"hook`":`"PreToolUse`",`"session_id`":`"$session`",`"cwd`":`"$($PWD.Path -replace '\\','\\\\')`",`"payload`":$input_text}"
...
try {
    ...
    $client.Dispose()
    Add-Content -Path $logPath -Value "$ts PreToolUse  -> ok" -ErrorAction SilentlyContinue
    ...
    Add-Content -Path $logPath -Value "$ts PreToolUse  -> err[$category]: $errMsg" -ErrorAction SilentlyContinue
    Add-Content -Path $errLog -Value "$ts PreToolUse $category $errMsg" -ErrorAction SilentlyContinue
    Write-Output "{}"
}
exit 0
