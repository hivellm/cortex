# cortex-user-prompt — UserPromptSubmit shim (Windows). Spec 10.
...
Add-Content -Path $logPath -Value "$ts UserPromptSubmit env_sid=$session payload_sid=$payloadSession pid=$PID" -ErrorAction SilentlyContinue
...
try {
    ...
    $client.Dispose()
    Add-Content -Path $logPath -Value "$ts UserPromptSubmit  -> ok" -ErrorAction SilentlyContinue
    ...
    Add-Content -Path $logPath -Value "$ts UserPromptSubmit  -> err[$category]: $errMsg" -ErrorAction SilentlyContinue
    Add-Content -Path $errLog -Value "$ts UserPromptSubmit $category $errMsg" -ErrorAction SilentlyContinue
    Write-Output "{}"
}
exit 0
