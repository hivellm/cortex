# cortex-notification — Notification shim (Windows). Spec 10.
...
Add-Content -Path $logPath -Value "$ts Notification env_sid=$session payload_sid=$payloadSession pid=$PID" -ErrorAction SilentlyContinue
$frame = "{`"hook`":`"Notification`",`"session_id`":`"$session`",`"cwd`":`"$($PWD.Path -replace '\\','\\\\')`",`"payload`":$input_text}"
...
try {
    ...
    $client.Dispose()
    Add-Content -Path $logPath -Value "$ts Notification  -> ok" -ErrorAction SilentlyContinue
    ...
    Add-Content -Path $logPath -Value "$ts Notification  -> err[$category]: $errMsg" -ErrorAction SilentlyContinue
    Add-Content -Path $errLog -Value "$ts Notification $category $errMsg" -ErrorAction SilentlyContinue
    Write-Output "{}"
}
exit 0
