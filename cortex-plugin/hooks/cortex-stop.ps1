...
Add-Content -Path $logPath -Value "$ts Stop env_sid=$session payload_sid=$payloadSession pid=$PID" -ErrorAction SilentlyContinue
...
try {
    ...
    $client.Dispose()
    Add-Content -Path $logPath -Value "$ts Stop  -> ok" -ErrorAction SilentlyContinue
    ...
    Add-Content -Path $logPath -Value "$ts Stop  -> err[$category]: $errMsg" -ErrorAction SilentlyContinue
    Add-Content -Path $errLog -Value "$ts Stop $category $errMsg" -ErrorAction SilentlyContinue
    Write-Output "{}"
}
exit 0
