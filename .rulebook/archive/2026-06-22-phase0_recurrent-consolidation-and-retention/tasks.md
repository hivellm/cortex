...
- [x] 4.1 DONE (2026-06-21): resolved via the LOCAL claude CLI, NOT the API (operator directive: never use the API, use the logged-in Claude Code CLI). Consolidator made CLI-only (commit 0701630) + classifier trigger producer enabled (`CORTEX_CONSOLIDATOR_TRIGGER_PRODUCER_ENABLED=true`, commit 58398b0) so it emits session_end/decision/topic triggers into Synap `cortex.consolidator.triggers`.
- [x] 4.2 DONE (2026-06-21): the consolidator runs OUTSIDE docker as a host daemon (`scripts/consolidator-daemon.cmd`) since it needs the host-local claude CLI; autostarts at logon via a Startup-folder `.vbs` (survives reboot). The in-container `consolidator_nightly` cron is moot (no claude in container; service moved behind the `manual` compose profile). No `ANTHROPIC_API_KEY` required. Documented in `docs/consolidator-host-daemon.md`.
- [x] 4.3 DONE (2026-06-21): backfill run live on the host — `cortex-consolidator nightly --dry-run false --all` (216 candidate sessions) summarising via `claude -p`, publishing consolidations OK to cortex-ingestion (verified event_ids, zero publish failures). Live ingest path also restored (phase0_live-ingestion-staleness §2). Going forward the host daemon consolidates new sessions in real time from the trigger stream.
...
- [x] 6.1 Update or create documentation covering the implementation — DONE (2026-06-21): CHANGELOG Added (coverage watchdog + cadence) + Fixed (archive_purge phantom lock_held + graph prune); `docs/specs/19-retention.md` gained "`--before` relative-duration shorthand" + "Coverage watchdog" sections.
- [x] 6.2 Write tests covering the new behavior — DONE (2026-06-21): retention windows (`parse_cutoff`/`parse_relative_duration`, 5 tests), scheduling (`seed_defaults` 14-job + watchdog row), watchdog evaluator (8 tests).
- [x] 6.3 Run tests and confirm they pass — DONE (2026-06-21): `cargo check --workspace` + `clippy --all-targets -D warnings` clean; `cargo test --workspace` = 3137 passed / 0 failed.
