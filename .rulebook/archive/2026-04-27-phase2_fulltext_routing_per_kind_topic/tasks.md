...
- [x] 5.1 Update or create documentation covering the implementation — `docs/specs/08-fulltext-indexer.md` §Routing matrix codifies the predicate→family table (with CODE_EXTENSIONS / DOC_EXTENSIONS allowlists, tie-break rule, and observability check)
- [x] 5.2 Write tests covering the new behavior — 7 routing unit tests (`crates/cortex-fulltext/src/routing.rs::tests`) + 1 mixed-batch integration test (`tests/indexer.rs::routing_matrix_distributes_mixed_batch_across_families`) asserting every family populated and `routed_total` mirrors `IndexReport.by_index`
- [x] 5.3 Run tests and confirm they pass — `cargo test -p cortex-fulltext` 38/38 (16 unit + 9 builders + 5 indexer + 3 routing + 9 worker passing); `cargo clippy -p cortex-fulltext --all-targets -- -D warnings` clean
