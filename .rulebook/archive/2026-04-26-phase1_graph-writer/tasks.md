...
- [x] 4.5 `OBSERVED_IN` (LawViolation → Turn|ToolCall) — tracked in follow-up `phase2_graph-observed-in-edge`; blocker is the upstream schema gap (`LawViolationPayload.observed_event_kind` discriminator), which the follow-up adds before the edge can land safely.
