...
- [ ] 1.1 `cortex_events_by_kind`: `cortex-api` HTTP route `POST /v1/search/events` accepting `{kind, repo?, session_id?, since?, until?, limit<=50}`; reads from the Meili kind-routed indexes; returns the native envelope shape (no fusion projection). MCP tool wraps it.
