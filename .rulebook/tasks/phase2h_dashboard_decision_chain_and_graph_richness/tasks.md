...
- [ ] 3.2 Cap total nodes at 60; surface `truncated: true` in the response when exceeded — partial: the dashboard caps at 50,000 (raised to fit the full panorama), but the response does NOT carry a `truncated` flag. The frontend tracks "shown / hidden leaf" by post-filtering `data.nodes` against the structural-skeleton kind set
- [ ] 3.4 Layout: keep the existing column-based x coordinates from the current handler; y coordinates spread evenly within each column (no explicit force layout) — abandoned: column layout dropped in favour of ForceAtlas2 (Sigma + graphology) because column placements did not scale past 60 nodes and the Sigma WebGL renderer handles 30k+ natively
- [ ] 3.5 Update `GraphPayload` shape: `{ nodes, edges, truncated: bool }` — abandoned: the structural-skeleton filter on the frontend tracks `shown / hidden` from `data.nodes` directly; no backend flag needed
...
- [ ] 4.4 Graph view shows a `truncated: showing first 60 of N nodes` banner when the flag is true — replaced by the side-panel `shown / edges / hidden N leaf` row in Graph.tsx (drives off the structural-skeleton filter result)
