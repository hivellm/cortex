...
- [ ] 2.2 Enable verifier + measure phantom-link rate; gate ≤1% (phase17 §3.10). FINDING (2026-06-22): the phantom-link verifier is the SAME dead-code bug as the reranker was — `with_verify` exists on the Orchestrator but cortex-api boot never calls it, so `verify_cfg`/`workspace_root` are never set and `apply_phantom_link_verification` never runs. Closing this needs: (a) a cortex-config field for the workspace root (ADR-016 forbids a direct `std::env::var` — the workspace audit gate would fail), since `apply_phantom_link_verification` resolves `workspace_root.join(snippet.path)` and the repo source IS mounted at `/workspaces/Cortex`; (b) wire `with_verify(cfg.verify, root)` in main.rs (mirror the reranker fix at main.rs:733); (c) a phantom-rate METRIC in cortex-eval (the suite currently measures MRR/recall, not phantom rate); (d) a cortex-api rebuild. Action defaults to "flag" (safe — annotates, never drops). Scoped but multi-step.
