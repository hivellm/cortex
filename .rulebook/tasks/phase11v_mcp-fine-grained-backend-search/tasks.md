...
- [x] 5.1 Documentation: spec 22 created; CHANGELOG updated; `docs/specs/22-fine-grained-search.md` enumerates wire reasons + status codes per failure path.
- [x] 5.2 Tests: 9 unit tests in `cortex-api::search_proxy::tests` (request serde defaults × 3, k/limit clamps × 3, ok_capped budget cap × 2, cypher_enabled env read × 1); registry-size-10 assertions pinned at `cortex-mcp-server::tools::tests::registry_returns_ten_tools_with_unique_names` and `cortex-mcp-server::server::tools_list_returns_ten_descriptors`. Hook-drift drift guard re-greened by syncing `cortex-plugin/hooks/`.
- [x] 5.3 Quality gates: `cargo check --workspace` clean; `cargo test -p cortex-api --lib search_proxy` 9/9 green; `cargo test -p cortex-mcp-server` 60+/60+ green (no failures). Pre-existing `cortex-cli` and `cortex-workers` clippy warnings remain — out of scope for phase11v.
...
- [ ] 5.5 ADR follow-up: lift the cypher gate decision into a new ADR once `CORTEX_GRAPH_CYPHER_ENABLED=1` is exercised by a real operator workflow. Today the gate is the right read-only default; an ADR is premature until non-trivial usage shape lands.
