...
## 7. Tail (mandatory)
- [x] 7.1 Update `docs/specs/06-embedder.md` status flag to 🟢 + index row
- [x] 7.2 Integration tests: 1 000-LOC Rust sample → correct symbol chunks; oversize summary substitution; idempotent replay (deduped == emitted); 429 soak; unknown language → fallback; schema-drift fail-fast
- [x] 7.3 Run `cargo check && cargo clippy -- -D warnings && cargo test`; coverage ≥95%
