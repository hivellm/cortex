...
- [ ] ⏸ 1.2 Run the payload-driven backfill (`--apply`) against the live graph; confirm SUPERSEDES / CONTRADICTS / EMITTED_BY / ANSWERED_BY / CITES edges land via `doctor-graph-coverage`. — Blocked on §1.3 (endpoint nodes). Finding from the bounded `--apply --limit 3000` run: under the StaticFallback classifier the full archive (32,485 envelopes) yields almost only EMITTED_BY (29,626/29,631) + 5 CITES; but EMITTED_BY's endpoint nodes `Tool[tool_name]` / `Model[model]` are NOT created by the mapper (it only mints `Tool` nodes from classifier "tool" entities, keyed differently), so every EMITTED_BY soft-drops → 0 persisted. Re-runnable once §1.3 lands the endpoint upserts.
