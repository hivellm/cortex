...
|---------------|--------------------------------------|-----------------------------------------------------|---------------------------------------------|--------|
| `vectorizer`  | `hivehub/vectorizer:3.6.1-fastembed` | `17001:15002` (REST), `17005:15503` (Thunder RPC)   | `vec-data:/data` (config), `vec-state:/.local/share/vectorizer` (real state — collections/vectors/auth) | `/health` — 3.6.1 added `ready`/`loading`/`state`, and `/collections` added `load_state`/`loaded_collections`/`expected_collections`, so a boot-time partial listing is no longer indistinguishable from data loss (vectorizer#391) |
| `nexus`       | `hivehub/nexus:3.0.0-alpha`          | `17002:15474` (REST), `17012:15475` (binary RPC)    | `nexus-data:/app/data`                      | `nexus-server --healthcheck` (exec-form; image is distroless, no shell). 3.0 refuses non-loopback bind without auth; compose sets `NEXUS_AUTH_REQUIRED_FOR_PUBLIC=false` (bridge-internal opt-out — auth enablement tracked as production hardening) |
