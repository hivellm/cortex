...
**Worker wiring.** `graph/worker.rs` runs the projection as batch step **3b** — after the mapper's structural patch (step 3) and the static-analyzer pass (step 3a), before `write_patches`. One `ExtractCtx::new(PROJECTION_ANALYZER_VERSION)` per batch pins a single `now_ms`. The two patch streams compose through the existing coalescer.

**Endpoint anchors (phase15c §1.3).** `project_envelope` appends one empty-props `NodeOp::with_identity` per distinct edge endpoint (both `from` and `to`, deduped, first-seen order so re-projection stays byte-identical). Without this, an edge to a label the mapper never mints — `Tool[tool_name]` / `Model[model]` for `EMITTED_BY`, or a referenced `Decision` / `Evidence` not in the batch — silently drops at the writer's `MATCH (a),(b) MERGE (a)-[r]->(b)` (the bounded backfill confirmed: 29,626 EMITTED_BY projected, 0 persisted pre-fix; 481/481 persisted post-fix). Empty props + `ConflictPolicy::Match` mean the coalescer (props union via `extend`) never clobbers a richer mapper/static node for the same `(label, natural_key)`.

**Projection kill-switch (phase15c).** The anchor-node + per-edge writes amplify write volume enough to trip Nexus 2.3.2's sustained-write stall (nexus#12) — a backlog-drain burst pins Nexus at 100% CPU until restarted, even structural-only. `CORTEX_GRAPH_PROJECTION_ENABLED` (`NexusConfig.projection_enabled`, default `true`) gates step 3b: set `false` to run the worker structural-mapping-only without a reverted binary. Held `false` in `docker-compose.yml` until Nexus can absorb the projection load.
...
**Coverage doctor.** `cortex-ops doctor-graph-coverage [--floor <share>] [--json]` queries `MATCH ()-[r]->() WHERE type(r) IN [<12 kinds>] RETURN type(r), count(r)`, renders a per-kind count + share table, and exits `1` if any kind has zero edges or falls below `--floor` (default `0.01` = 1%), `2` if Nexus is unreachable. `cortex-ops graph backfill --since <RFC3339> [--apply --limit <N>]` re-projects archived envelopes — count-only dry-run by default, or `--apply` to write a bounded window through the real `GraphWriter` (payload-driven kinds only under the StaticFallback classifier).
...
> **✓ Persistence works as of Nexus 2.3.4 (hivellm/nexus#25).** Nexus
> 2.3.2 silently dropped inline relationship properties on `MERGE` (the
> writer's `render_edge_merge` form), so `confidence` / the provenance
> triple read back null. **nexus#25, fixed in 2.3.4, persists inline rel
> props on MERGE** (and supports `SET` on a rel variable) — verified live
> 2026-06-21: `MERGE (a)-[r {confidence:"ambiguous"}]->(b)` reads back
> `ambiguous` (was null). No writer change was needed; `render_edge_merge`'s
> existing inline-props output now round-trips end-to-end on 2.3.4. Edges
> written before 2.3.4 still carry no props until rewritten. Closed in
> `phase0_nexus-merge-drops-edge-props`.
...
9. **SCIP precise edges supersede heuristic extraction where they cover a file (phase27d).** `graph/scip.rs` parses `rust-analyzer scip` output (protobuf — a minimal hand-rolled wire reader, no protoc build dependency; the real-fixture schema is pinned in `phase27d`'s design.md + `tests/fixtures/scip/`) and emits function-level `:Symbol` nodes (keyed `repo|rust|qualified_name`, same convention as the tree-sitter analyzer) plus exact-target `DEFINES`/`CALLS`/`REFERENCES` edges at `EdgeConfidence::Extracted` (1.0), prop `analyzer="scip"`. Cross-crate / unresolved targets land on `:ScipExternal` stub nodes (keyed `scheme|manager|package|descriptors`) so edges never dangle. Bootstrap/CI invocation wiring is pending an operator decision on the rust-analyzer tool dependency (phase27d §2.4).
10. **Community membership is a node property, not a new label or edge type (phase27b).** `community_id` (u32), `community_level` (u32, Leiden hierarchy level), and `is_god_node` (bool, hub-percentile-excluded then reattached) are stamped onto existing architecture nodes by `community_node_ops()` (`crates/cortex-workers/src/graph/community.rs`) via the same idempotent `NodeOp` surface as every other writer — no new node/edge kind, no schema migration. The writeback worker that actually runs this against the live graph is gated on the semantic graph projection (ADR-027, tracked separately); until that lands, no live node carries these properties. See spec 45 (graph communities) for the read-side query/tool/dashboard surface built against this property.
